...
% FRHF inertial: Inertial forward reflected half forward splitting for image restoration
%Inertial alpha>0
% Inputs:
...
ulam = (1-lam);%1-relaxation parameter
ulam2 = (1-lam)*gam;
...
if a > 0 && lam > 0 && lam ~= 1
    ...
    while error > tol & iter<maxiter
        ...
        xx1 = p21 + a3*x2_1 - a2*xo2_1 + a*xoo2_1;
        xx2 = p22 + a3*x2_2 - a2*xo2_2 + a*xoo2_2;
  
        q1 = y1 - gam*(L1T(xx1)+L2T(xx2))-C(y1);
    
        qq=p1 + a3*x1_ - a2*xo1_ + a*xoo1_;
        qq1 = L1(qq);
        qq2 = L2(qq);
        ...
        p1 = max(min(q1,255),0);%Projection
    
        

        xx2 = abs(q21) -  lams; 
        p21 = gam*(q21-sign(q21).*((xx2>0).*xx2));  %Prox conjugate l1 norm

        xx2 = abs(q22) -  lams;      
        p22 = gam*(q22-sign(q22).*((xx2>0).*xx2));  %Prox conjugate l1 norm
...
elseif a == 0 && lam > 0 && lam ~= 1
    ...
    while error > tol & iter<maxiter
        ...
        x1 = ulam*x1_ + lam*p1;%relaxation step
        x21 = ulam*x2_1 + lam*p21;%relaxation step
        x22 = ulam*x2_2 + lam*p22;%relaxation step
        % --- Compute relative primal dual error 
        error = sqrt((norm(x1(:)-x1_(:))^2+norm(x21(:)-x2_1(:))^2+norm(x22(:)-x2_2(:))^2)/(norm(x1_(:))^2+norm(x2_1(:))^2+norm(x2_2(:))^2));
    end
    % Record total execution time 
...
elseif a > 0 && lam == 1
        ...
        while error > tol & iter<maxiter
         ...
         qq1 = L1(qq);
         qq2 = L2(qq);
         q21 = a3g*x2_1 - ag*xo2_1 + qq1;%inertial step
         q22 = a3g*x2_2 - ag*xo2_2 + qq2;%inertial step
